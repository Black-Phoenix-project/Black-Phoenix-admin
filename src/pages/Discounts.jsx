...
const Discounts = () => {
  ...
  const submit = async (e) => {
    ...
    if (!form.title.trim() || form.value <= 0) {
      toast.error("Sarlavha va miqdor majburiy");
      return;
    }
    setSubmitting(true);
    try {
        ...
        <section className="relative overflow-hidden rounded-3xl border border-warning/30 bg-base-200 px-6 py-7 shadow-lg">
          ...
          <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-bold text-warning">Chegirmalar</h1>
                <p className="mt-0.5 text-sm text-base-content/60">Aksiyalar va chegirmalarni boshqaring</p>
              </div>
            </div>
            <button
              type="button"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <section className="lg:col-span-2 rounded-3xl border border-warning/20 bg-base-100 p-5 md:p-6 shadow-md flex flex-col gap-4">
            <h2 className="text-lg font-bold text-warning">
              {editingId ? "Chegirmanni tahrirlash" : "Yangi chegirma"}
            </h2>
            <form className="flex flex-col gap-3" onSubmit={submit}>
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-base-content/60 uppercase tracking-wider">Sarlavha</label>
                <input
                  name="title"
                  value={form.title}
                  onChange={onChange}
                  placeholder="Masalan: Birinchi buyurtmaga 20%"
                  className="input input-bordered bg-base-200 border-warning/25 focus:border-warning focus:outline-none w-full"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-semibold text-base-content/60 uppercase tracking-wider">Turi</label>
                  <select
                    name="type"
                    value={form.type}
                    onChange={onChange}
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-semibold text-base-content/60 uppercase tracking-wider">Miqdor</label>
                  <input
                    type="number"
                    name="value"
                    value={form.value}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-base-content/60 uppercase tracking-wider">Doira</label>
                <select
                  name="scope"
                  value={form.scope}
                  onChange={onChange}
              ...
              {form.scope === "product" && (
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-semibold text-base-content/60 uppercase tracking-wider">Mahsulot</label>
                  <select
                    name="productId"
          ...
          <section className="lg:col-span-3 rounded-3xl border border-warning/20 bg-base-100 p-5 md:p-6 shadow-md flex flex-col gap-4">
            <h2 className="text-lg font-bold text-warning">Barcha chegirmalar</h2>
            {loading ? (
              ...
              <div className="rounded-2xl border-2 border-dashed border-warning/25 bg-base-200/40 flex flex-col items-center justify-center text-center px-4 gap-3 py-16">
                ...
                <p className="font-bold text-base-content">Chegirmalar yo'q</p>
              </div>
            ) : (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-base-content/55">
                        {item.type === "percent" ? `%${item.value}` : `${item.value} so'm`} ·{" "}
                        {item.scope === "product" ? "mahsulot" : "umumiy"}
                      </p>
                      <p className="text-[11px] text-base-content/40 mt-0.5">
                        {item.active === false ? "no faol" : "faol"}
                      </p>
